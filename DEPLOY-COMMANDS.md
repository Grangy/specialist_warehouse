...
## 🧹 Пометка существующих заказов как интегрированных

После первого деплоя endpoint sync-1c нужно пометить все существующие обработанные заказы как уже интегрированные в 1С, чтобы они не попадали в ответ:
...
# Вариант 3: Напрямую через SQL (если нужно)
...
**Что делает скрипт:**
- Находит все заказы со статусом `processed` и `exportedTo1C = false`
- Помечает их как интегрированные (`exportedTo1C = true`)
- Устанавливает время интеграции (`exportedTo1CAt`)
- После этого только новые заказы будут возвращаться в endpoint sync-1c
...
- **Пометка существующих заказов** - выполните после первого деплоя sync-1c endpoint
