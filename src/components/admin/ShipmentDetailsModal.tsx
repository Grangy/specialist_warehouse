...
export default function ShipmentDetailsModal({ shipmentId, onClose, canReassign = false }: ShipmentDetailsModalProps) {
  ...
  const handleAssemblyError = async (taskId: string, lineIndex: number, lineName: string) => {
    if (
      !details ||
      !window.confirm(
        `Зафиксировать ошибку проверки по позиции «${lineName}»? Сборщик −1/−5 (новенький/остальные), проверяльщик −10, админ +11/+15.`
      )
    )
      return;
                                            ...
                                            {canReassign && (
                                              <td className="py-1.5 px-2 text-right">
                                                <button
                                                  ...
                                                  title="Ошибка проверки: сборщик −1/−5, проверяльщик −10, админ +11/+15"
                                                >
