...
export async function POST(
  ...
  try {
    ...
    if (areAllTasksConfirmed(allTasks)) {
      ...
      // ВАЖНО: Всегда используем сумму мест из заданий как основу
      // Места из модального окна используются только если они явно указаны и отличаются от суммы
      // Если места не указаны в модальном окне (undefined), используем сумму из заданий
      // Если места указаны и равны сумме из заданий, используем сумму из заданий (не дублируем)
      // Если места указаны и отличаются от суммы, используем значение из модального окна (пользователь скорректировал)
      let finalPlaces: number;
      if (places !== undefined && places > 0 && places !== totalPlacesFromTasks) {
        // Пользователь скорректировал значение в модальном окне
        finalPlaces = places;
        ...
        console.log(`[API Confirm]   - Места из модального окна (скорректировано): ${places}`);
        console.log(`[API Confirm]   - ИТОГО мест: ${finalPlaces} (используется скорректированное значение)`);
      } else {
        // Используем сумму мест из заданий (по умолчанию)
