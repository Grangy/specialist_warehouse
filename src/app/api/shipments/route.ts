  ...
  try {
    ...
    for (const shipment of shipments) {
      ...
      for (const task of shipment.tasks) {
        ...
        // ВАЖНО: роль warehouse_3 должна видеть задачи всех складов (Склад 1/2/3) как проверяльщик.
        // Ограничения по складу применяются только в действиях (например lock на "Склад 3"),
        // а не в отображении списка.
        // Для режима ожидания показываем все задания (включая processed)
