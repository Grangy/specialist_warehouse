...
## Обновление только отдельных блоков (без перезагрузки списка)

Логика в `useShipments.ts` и на странице настроена так, чтобы не перезагружать весь список:

- **lock/unlock** — по SSE обновляется только нужное задание (ответственный, флаг locked).
- **shipment:updated** — по SSE обновляется только одно задание (статус, например переход в «ожидание подтверждения»).
- **shipment:status_changed** с `status: 'processed'` — все задания этого заказа **удаляются из state** (блоки исчезают без полной перезагрузки). При другом статусе — только обновление поля статуса у заданий этого заказа.
- После **подтверждения** и **закрытия модалок** сборки/проверки полный `refreshShipments()` не вызывается — список подтягивается по SSE. Полная перезагрузка остаётся только после сброса прогресса, удаления сборки и при ошибке отправки в офис (и по кнопке «Обновить»).

Так список не «прыгает» и не перерисовывается целиком при каждом действии.

## Внесённые изменения

1. **useShipments.ts** — удалён интервал резервного автообновления раз в 1 минуту. Список заказов обновляется только при первой загрузке, по событиям SSE и по кнопке «Обновить» (и после сброса/удаления/ошибки).
2. **useShipments.ts** — при SSE `shipment:status_changed` с `status: 'processed'` задания этого заказа удаляются из state (блоки исчезают без refetch).
3. **page.tsx** — убраны вызовы `refreshShipments()` после подтверждения и при закрытии модалок сборки/проверки; при «отправке в офис» (частичное подтверждение) полная перезагрузка не вызывается, при ошибке — вызывается.
4. **Header.tsx** — интервал обновления баллов в шапке увеличен с 30 до 60 секунд.
