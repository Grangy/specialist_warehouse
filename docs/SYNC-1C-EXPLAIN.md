...
1. **1С дергает нас:** `POST /api/shipments/sync-1c` с телом `{ orders: [ { id?, number?, customer_name?|customer?, success } ] }`. Идентификация заказа: по **id**, затем по **number + customer_name** (или customer), затем по **number** (id в 1С может отличаться при каждой выгрузке).
...
4. **Заказа нет у нас в БД (удалён вручную и т.п.), а 1С ещё шлёт** — 1С ставит `success: true` только когда получает заказ в нашем ответе «готовые к выгрузке». Поэтому в массив **`orders`** ответа добавлены минимальные объекты `{ id }` по каждому такому заказу (список в `success_ids`). 1С получает их в ответе, считает выгрузку успешной и перестаёт присылать их с `success: false`. Поле **`success_ids`** в ответе дублирует список для отладки.
...
**Уже выгруженный заказ:** если 1С присылает заказ с success: true, а у нас он уже помечен exportedTo1C, в ответ добавляется массив **`errors`**: `[{ number, customer_name, error: "already_exported" }]` — чтобы 1С воспринял это как ошибку (как при активном заказе) и перестал слать.

**Логи sync-1c:** для каждого заказа — по какому ключу найден (id / number+customer / number), найден ли, уже выгружен ли; итог: сколько уже выгружены (ошибка в ответе), сколько не найдено в БД.
