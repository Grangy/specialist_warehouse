...
**Уже выгруженный заказ:** если 1С присылает заказ с success: true, а у нас он уже помечен exportedTo1C, мы его не обновляем и **не включаем в ответ** — отдаём только массив **`orders`** (готовые к выгрузке). Уже выгруженные в ответ не попадают.
