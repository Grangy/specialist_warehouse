...
3. **Автоматически в Sync-1C** — с текущей версией любой заказ из запроса 1С (и с `success: true`, и с `success: false`), если он есть в БД со статусом processed и не удалён, помечается как выгруженный. После следующего вызова 1С эти заказы перестанут отдаваться в ready-for-export.
