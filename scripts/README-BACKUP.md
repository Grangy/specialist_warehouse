...
- **Планировщик** (`npm run db:backup:scheduled`):
  ...
  - `backups/5h/` — раз в 5 часов та же копия дополнительно сохраняется сюда, хранятся **5** копий.
...
## Планировщик бэкапов (каждые 30 мин + 5 копий по 5 ч)

Долгоиграющий процесс: бэкап каждые 30 минут, хранение 10 копий в `backups/30m/`; раз в 5 часов та же копия пишется в `backups/5h/`, хранятся 5 копий. При старте скрипта лишние старые файлы удаляются.
