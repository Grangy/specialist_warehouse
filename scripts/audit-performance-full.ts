...
async function main() {
  ...
  add('|----------|----------|------------|');
  add('| ShipmentsPollingContext | 18 сек (backoff до 120 сек) | Опрос изменений + полный refetch списка при hasUpdates |');
  add('| ExtraWorkContext | 45 сек без сессии / 5 сек при активной сессии | Доп. работа, таймеры |');
  add('| useCollect heartbeat | 8 сек при открытой сборке | Удержание блокировки заказа |');
  add('');
  add('> Оценка частоты (грубо): опрос заказов ~N_вкладок/18 с; доп. работа ~N/45 или N/5; heartbeat ~N_сборок/8 с. Не снижать интервалы без необходимости.');
  add('');
