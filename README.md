...
- ✅ **Система ролей** - Четыре роли: администратор, сборщик, проверяющий, Склад 3
...
- ✅ **Склад 3** - Роль с доступом к сборке и проверке (диктовщик по умолчанию — сам пользователь), активные сборки по регионам
...
| 📦 Сборщик | `collector` | `YOUR_PASSWORD` | Только "Новые" заказы |
| ✅ Проверяющий | `checker` | `YOUR_PASSWORD` | "Новые", "На руках", "Подтверждения", "Регионы" |
| 📍 Склад 3 | `warehouse_3` | `YOUR_PASSWORD` | Как проверяющий + только задания Склад 3, диктовщик по умолчанию — сам себе |
...
**Доступ к разделам "Новые", "На руках", "Подтверждения", "Регионы":**
- ✅ Просмотр новых заказов и заказов на руках
- ✅ Просмотр заказов, ожидающих подтверждения
- ✅ Подтверждение собранных заказов (с выбором диктовщика)
- ✅ Активные сборки по регионам
- ✅ Перевод заказов в статус "Обработан"

### 📍 Склад 3 (`warehouse_3`)

**Доступ как у проверяющего, с ограничением по складу и упрощённым диктовщиком:**
- ✅ Те же вкладки: Новые, На руках, Подтверждения, Регионы (активные сборки по регионам)
- ✅ Только задания склада «Склад 3»
- ✅ Сборка и отправка на проверку, подтверждение заказов
- ✅ Диктовщик по умолчанию — сам пользователь (все баллы проверки себе)
- ✅ Админ-панель с фильтром по Склад 3 (завершённые заказы, статистика, позиции)
...
> **Примечание:** Все endpoints (кроме `/auth/login`, `/auth/session`, `/api/statistics/top` и `/api/statistics/user/[id]/public`) требуют авторизации через cookie `session_token`.
